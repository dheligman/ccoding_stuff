...
int main() {
    ...
    const auto result1 = mean<double>(nums_d, sizeof(nums_d)/sizeof(double));
    printf("double: %f\n", result1);

    const float nums_f[]{ 1.0f, 2.0f, 3.0f, 4.0f };
    const auto result2 = mean<float>(nums_f, sizeof(nums_f)/sizeof(float));
    printf("float: %f\n", result2);

    const size_t nums_c[] { 1, 2, 3, 4 };
    const auto result3 = mean<size_t>(nums_c, sizeof(nums_c)/sizeof(size_t));
    printf("size_t: %zu\n", result3);
}